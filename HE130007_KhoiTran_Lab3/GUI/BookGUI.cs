...
namespace HE130007_KhoiTran_Lab3.GUI
{
    public partial class BookGUI : Form
    {
        ...
        {
            InitializeComponent();
            //DataTable bdt = BookDAO.GetDataTable();
            //dataGridView1.DataSource = bdt;
            //DataTable cdt = CopyDAO.GetDataTable();
        ...
        private void btnFiilter_Click(object sender, EventArgs e)
        {
            int bookNumber;
            try
            {
            ...
            if (bookNumber > -1)
            {
                dv.RowFilter = "BookNumber = " + bookNumber.ToString();
            }
        }
      ...
      //Add is not done  
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            DialogResult dr = MessageBox.Show(String.Format("Do you want to delete this book number {0}", bookNumber), "Confirm deleting", MessageBoxButtons.YesNo);
        ...
        private void dataGridView2_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            txtCopyBookNumber.Text =
                dataGridView2.Rows[e.RowIndex].Cells["bookNumber"].Value.ToString();
            txtCopyNumber.Text =
                dataGridView2.Rows[e.RowIndex].Cells["copyNumber"].Value.ToString();
            txtSequenceNumber.Text =
                dataGridView2.Rows[e.RowIndex].Cells["sequenceNumber"].Value.ToString();
            txtType.Text = dataGridView2.Rows[e.RowIndex].Cells["type"].Value.ToString();
            txtPrice.Text =
                dataGridView2.Rows[e.RowIndex].Cells["price"].Value.ToString();
        }
        ...
        private void txtType_Validating(object sender, CancelEventArgs e)
        {
            ...
            {
                //Cacel the event and select the text to be concerned by the user/
                e.Cancel = true;
                txtType.Select(0, txtType.Text.Length);

                //Set the ErroPrivder erro with the text to display.
